...
public class Item_Edit_Extra {
    public void Bring_Item_Add_Screen_Extra(Context context, View root, @Nullable ViewGroup[] relations) {
        ViewGroup item_purpose_container = root.findViewById(R.id.invoice_item_card_editing_content_calculate_type);
        ViewGroup item_buy_type_container = root.findViewById(R.id.invoice_item_card_editing_content_buy_type);
        ViewGroup item_buyer_container = root.findViewById(R.id.invoice_item_card_editing_content_buyer_share_type);

        //PURPOSE CONTAINER
        for (byte i = 0; i < Types.Item_Calculate_Types.values().length; i++) {
            Button new_item_purpose_list_button = new Button(context);
            new_item_purpose_list_button.setText(Types.Item_Calculate_Types.values()[i].name());
    ...
    public ViewGroup Bring_Purpose_User_List(Context context, View root, List<User> user_list) {
        ViewGroup v = root.findViewById(R.id.invoice_item_card_editing_content_calculate_type_related_users);
    ...
    public ViewGroup Bring_User_List(Context context, View root, List<User> user_list) {
        ViewGroup parent = root.findViewById(R.id.invoice_item_card_editing_content_buyers);
